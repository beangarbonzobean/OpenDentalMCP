...
def get_tools_definitions() -> List[Dict[str, Any]]:
    """Tool list for tools/list — same descriptions and inputSchemas as prior MCP Tool definitions."""
    return [
        {
            "name": "list_tables",
...
class DEXISMCPServer:
    ...
    def handle_tools_list(self):
        ...
        self.send_response({"tools": get_tools_definitions()})

    def handle_tool_call(self, params: Dict):
        """Call a tool and return MCP content wrapper."""
        ...
        try:
            ...
            if tool_name == "list_tables":
                ...
                result = describe_table(arguments.get("table_name"), cfg)
            elif tool_name == "list_columns":
                result = list_columns(arguments.get("table_name"), cfg)
                ...
                result = search_patient(arguments.get("name"), cfg)
                ...
                result = get_xrays_by_date(arguments.get("target_date"), cfg)
                ...
                result = get_xray_info(arguments.get("visual_id"), cfg)
            elif tool_name == "get_recent_xrays":
                result = get_recent_xrays(arguments.get("limit", 50), cfg)
